...
| What you will learn               | How to declare and use atomic_fence in a DPC++ program
...
## Building the `atomic_fence` Tutorial
...
3. (Optional) As the above hardware compile may take several hours to complete,
   ...
   downloaded <a
   href="https://iotdk.intel.com/fpga-precompiled-binaries/latest/atomic_fence.fpga.tar.gz"
   download>here</a>.
...
Locate `report.html` in the `atomic_fence_report.prj/reports/` directory. Open
the report in any of Chrome*, Firefox*, Edge*, or Internet Explorer*.

## Running the Sample

 1. Run the sample on the FPGA emulator (the kernel executes on the CPU):
     ```
     ./atomic_fence.fpga_emu     (Linux)
     atomic_fence.fpga_emu.exe   (Windows)
     ```
2. Run the sample on the FPGA device:
     ```
     ./atomic_fence.fpga         (Linux)
     ```
